    ...
    if(payload) {
        ...
        function onChange() {
            ...
            if(error) {
                ...
                `${error.message}${hint && `: <span style="color: black">${hint.slice(0, hint.length-1)}</span><span style="font-weight: bold">${hint[hint.length-1]}</span>`}`;
        ...
        nestButton.onclick = function() {
            payload.value = pretify(JSON.stringify({
